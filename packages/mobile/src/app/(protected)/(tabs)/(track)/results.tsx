...
export default function ResultsSubmissions() {
  const { userId } = useSupabase();
  const { bottom } = useSafeAreaInsets();
  const { data, fetchNextPage, hasNextPage, isPending } = useInfiniteQuery(
    userSubmissionEventsInfiniteQuery(userId, { status: "completed", pageSize: 20 }),
  );
  ...
  if (events.length === 0) {
    return (
      <ThemedView style={styles.empty}>
        <ThemedText opacity={60}>No results yet.</ThemedText>
      </ThemedView>
    );
  }
